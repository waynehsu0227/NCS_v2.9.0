...
//rex add start
static uint16_t report_interval_us = CONFIG_DESKTOP_MOTION_SENSOR_POLLING_INTERVAL_MS * 1000; // [ms]
//rex add end
enum state {
	STATE_DISABLED,
...
//rex add start. polling timer
static volatile bool             motion_polling_timer_running = false;
static void motion_polling_expire(struct k_timer *timer)
{
...
K_TIMER_DEFINE(motion_polling_timer, motion_polling_expire, NULL);
//rex add end. polling timer
...
//rex add start. polling timer
			if (IS_ENABLED(CONFIG_DESKTOP_MOTION_SENSOR_POLLING_ENABLE)) {
				if (motion_polling_timer_running){
					k_timer_stop(&motion_polling_timer);
					motion_polling_timer_running = false;
				}
			}
//rex add end. polling timer
			err = 0;
...
{
	set_option(event->opt ,event->value);
	return false;
}

#ifdef CONFIG_DPM_ENABLE
static bool handle_dpm_set_report_rate_event(const struct dpm_set_report_rate_event *event)
{
    report_interval_us = (event->report_interval) * 1000;
    return false;
}
#endif
		...
		const struct hid_report_sent_event *event =
			...
			if ((event->report_id == REPORT_ID_MOUSE) ||
				...
				if (state.state == STATE_FETCHING) {
					...
					{
        				if (!motion_polling_timer_running){
							motion_polling_timer_running = true;
							k_timer_start(&motion_polling_timer, K_NO_WAIT, K_USEC(report_interval_us));
        				}					
					}
					else{
						state.sample = true;
...
//rex add end. polling timer
	...
	if (is_hid_report_subscription_event(aeh)) {
		...
		if ((event->report_id == REPORT_ID_MOUSE) ||
		    (event->report_id == REPORT_ID_BOOT_MOUSE)) {
			LOG_ERR("event->enabled :%d ,state.peer_count:%d",event->enabled,state.peer_count);
			if (event->enabled) {
...
#ifdef CONFIG_DPM_ENABLE
    if (is_dpm_set_report_rate_event(aeh)) {
		return handle_dpm_set_report_rate_event(cast_dpm_set_report_rate_event(aeh));
	}
#endif
...
#endif
#ifdef CONFIG_DPM_ENABLE
APP_EVENT_SUBSCRIBE(MODULE, dpm_set_report_rate_event);
#endif
