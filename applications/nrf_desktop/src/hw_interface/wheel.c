...
#define NRF_DESK_QDEC DT_NODELABEL(qdec20)

/* obtan qdec pins from default state */
static const uint32_t qdec_pin[] = {
	DT_FOREACH_CHILD_VARGS(
		DT_PINCTRL_BY_NAME(NRF_DESK_QDEC, default, 0),
...
static const struct device *qdec_dev = DEVICE_DT_GET(NRF_DESK_QDEC);
...
static const struct device *map_gpio_port(int pin_number)
{
	...
	case 2:
		dev = DEVICE_DT_GET_OR_NULL(DT_NODELABEL(gpio2));
		break;
	default:
		break;
	}
...
{
	int enable_pin = DT_PROP(NRF_DESK_QDEC, enable_pin);
