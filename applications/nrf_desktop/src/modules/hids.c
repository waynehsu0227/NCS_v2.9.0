...
#if CONFIG_MOTION_SENSOR_CONFIG_EVENTS
...
static void dpm_input_report_sent_cb(struct bt_conn *conn, void *user_data)
{
	ARG_UNUSED(user_data);
	hid_report_sent(conn, REPORT_ID_DPM_INPUT, false);
}
static void dpm_input_notif_handler(enum bt_hids_notify_evt evt)
{
	async_notif_handler(REPORT_ID_DPM_INPUT, evt);
}
#endif
...
#ifdef CONFIG_DPM_ENABLE
static void dpm_feature_report_handler(struct bt_hids_rep *rep,
				   struct bt_conn *conn,
				   bool write)
{
    extern void dpm_get_feature_report(uint8_t *report_val, uint8_t size);
    extern void dpm_set_feature_report(uint8_t *report_val, uint8_t size);

    if (!write) {
        dpm_get_feature_report(rep->data, rep->size);
    } else {
         dpm_set_feature_report(rep->data, rep->size);
    }
}
#endif
...
static int module_init(void)
{
    ...
    if (IS_ENABLED(CONFIG_DPM_ENABLE)) {
        static const uint8_t mask[] = REPORT_MASK_DPM_INPUT;
		BUILD_ASSERT((sizeof(mask) == 0) ||
			     (sizeof(mask) == DIV_ROUND_UP(REPORT_SIZE_DPM_INPUT, 8)));
		BUILD_ASSERT(REPORT_ID_SYSTEM_CTRL < ARRAY_SIZE(report_index));
		input_report[ir_pos].id       = REPORT_ID_DPM_INPUT;
		input_report[ir_pos].size     = REPORT_SIZE_DPM_INPUT;
		input_report[ir_pos].handler  = dpm_input_notif_handler;
		input_report[ir_pos].rep_mask = (sizeof(mask) == 0)?(NULL):(mask);
...
#ifdef CONFIG_DPM_ENABLE
    if (IS_ENABLED(CONFIG_DPM_ENABLE)) {
		feature_report[feat_pos].id      = REPORT_ID_DPM_FEATURE;
		feature_report[feat_pos].size    = REPORT_SIZE_DPM_FEATURE;
		feature_report[feat_pos].handler = dpm_feature_report_handler;
...
#ifdef CONFIG_DPM_ENABLE
        [REPORT_ID_DPM_INPUT] = dpm_input_report_sent_cb,
#endif
	};
...
#if (CONFIG_DPM_ENABLE && CONFIG_MOTION_SENSOR_CONFIG_EVENTS)
static bool handle_motion_sensor_config_event(const struct motion_sensor_config_event *event)
{
    if (event->opt != MOTION_SENSOR_OPTION_CPI)
        return false;
    uint16_t value = event->value;
    uint8_t buf[] = {0x0C,
                    (uint8_t)(value & 0x00ff),
                    (uint8_t)(value >> 8)};
    int err = bt_hids_inp_rep_send(&hids_obj, cur_conn,
                    REPORT_ID_DPM_INPUT,
                    buf, sizeof(buf),
                    dpm_input_report_sent_cb);
    if (err) {
		if (err == -ENOTCONN) {
			LOG_WRN("Cannot send report: device disconnected");
		} else if (err == -EBADF) {
...
//		hid_report_sent(cur_conn, EPORT_ID_DPM_INPUT, true);
	}
	return false;
}
#endif
...
#if (CONFIG_DPM_ENABLE && CONFIG_MOTION_SENSOR_CONFIG_EVENTS)
    if (IS_ENABLED(CONFIG_MOTION_SENSOR_CONFIG_EVENTS) &&
	    is_motion_sensor_config_event(aeh)) {
		return handle_motion_sensor_config_event(cast_motion_sensor_config_event(aeh));
	}
#endif
...
#if CONFIG_MOTION_SENSOR_CONFIG_EVENTS
APP_EVENT_SUBSCRIBE(MODULE, motion_sensor_config_event);
#endif
