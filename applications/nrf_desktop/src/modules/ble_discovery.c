...
#if CONFIG_DPM_ENABLE
//billy+++
static const struct bt_uuid * const pnp_uuid = BT_UUID_GAP_DEVICE_NAME;//BT_UUID_DIS_PNP_ID;
#else
...
#if CONFIG_DPM_ENABLE
//billy+++
static void read_device_name(const uint8_t *ptr, uint16_t len)
{

	memcpy(host_name, ptr, len);
	...
	if (err > 0) {
		LOG_INF("device name: %s", log_buf);
	} else {
		LOG_ERR("Failed to log device name");
	}

    struct dpm_save_host_name_event *event =
		new_dpm_save_host_name_event(len);
    memcpy(event->dyndata.data, ptr, len);
    APP_EVENT_SUBMIT(event);
}
//billy===
#endif
...
#if CONFIG_DPM_ENABLE
//billy+++
	case DISCOVERY_STATE_GAP:
		read_device_name(data_ptr,length);
		break;
//billy===
#endif
	default:
...
//billy+++
	case DISCOVERY_STATE_GAP:
		start_discovery(discovering_peer_conn, BT_UUID_GAP, false);
		break;
#endif
