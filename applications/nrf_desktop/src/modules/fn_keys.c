...
#if CONFIG_CY25
static bool fn_lock_active = true;
#else
...
#if CONFIG_CY25
static bool is_fn_no_lock_key(uint16_t key_id)
{
	for (int i = 0; i < ARRAY_SIZE(fn_no_lock_keys); i ++) {
		if (key_id == fn_no_lock_keys[i]) 
			return true;
	}
	return false;
}
...
static bool button_event_handler(const struct button_event *event)
{
	...
	if (unlikely(event->key_id == CONFIG_DESKTOP_FN_KEYS_LOCK)) {
		if (event->pressed) {
			...
			LOG_ERR("fn_lock:%d", fn_lock_active);
			store_fn_lock();
		}
		return true;
	}
	...
	bool fn_lock_trigger = (!is_fn_no_lock_key(event->key_id) && fn_lock_active);
	bool fn_active = (fn_switch_active != fn_lock_trigger);
#else
