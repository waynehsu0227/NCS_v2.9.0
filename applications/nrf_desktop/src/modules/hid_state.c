...
#include "hid_event.h"
#include "battery_event.h"//aw
#include "dpm_config.h"//aw
...
#define ITEM_COUNT MAX(MAX(MOUSE_REPORT_BUTTON_COUNT_MAX,	\
			   ...
			   MAX(CONSUMER_CTRL_REPORT_KEY_COUNT_MAX,	\
               DPM_INPUT_REPORT_COUNT_MAX)))
#else
#define ITEM_COUNT MAX(MAX(MOUSE_REPORT_BUTTON_COUNT_MAX,	\
			   KEYBOARD_REPORT_KEY_COUNT_MAX),	\
		       MAX(SYSTEM_CTRL_REPORT_KEY_COUNT_MAX,	\
...
static void send_report_mouse(struct report_state *rs, struct report_data *rd)
{
	...
	/* Encode report. */
#if CONFIG_DESKTOP_TILT_BUTTONS_ENABLE
	BUILD_ASSERT(REPORT_SIZE_MOUSE == 6, "Invalid report size");
#else
...
#if CONFIG_DESKTOP_TILT_BUTTONS_ENABLE
	event->dyndata.data[6] = rd->axes.axis[MOUSE_REPORT_AXIS_TILT];
#endif
	APP_EVENT_SUBMIT(event);
...
#ifdef CONFIG_DPM_ENABLE
static void send_report_dpm_input(struct report_state *rs, struct report_data *rd)
{
#if 0 
	__ASSERT_NO_MSG(rs->report_id == REPORT_ID_DPM_INPUT);

	if (!IS_ENABLED(CONFIG_DPM_ENABLE)) {
		/* Not supported. */
		__ASSERT_NO_MSG(false);
		return;
	}
	struct hid_report_event *event = new_hid_report_event(sizeof(rs->report_id)
							                            + REPORT_SIZE_DPM_INPUT);
    memset(event->dyndata.data, 0, event->dyndata.size);
	event->source = &state;
	event->subscriber = rs->subscriber->id;
	event->dyndata.data[0] = rs->report_id;
    event->dyndata.data[1] = 0x0B;

	const size_t idx = ARRAY_SIZE(rd->items.item) - 1;
	printk("REPORT_ID_DPM_INPUT rd->items.item[idx].usage_id=%d, event->dyndata.size=%d\n",rd->items.item[idx].usage_id,event->dyndata.size);
	sys_put_le16(rd->items.item[idx].usage_id,
		     &event->dyndata.data[sizeof(rs->report_id) + 1]);
    LOG_HEXDUMP_INF(event->dyndata.data, event->dyndata.size,__func__);
	APP_EVENT_SUBMIT(event);

	rs->update_needed = false;
#else //aw  Building a temporary battery indicator
	...
	struct hid_report_event *event = new_hid_report_event(sizeof(rs->report_id)+ REPORT_SIZE_DPM_INPUT);
	
    memset(event->dyndata.data, 0, event->dyndata.size);
	event->source = &state;
	event->subscriber = rs->subscriber->id;
	event->dyndata.data[0] = rs->report_id;
    event->dyndata.data[1] = 0x08;
	event->dyndata.data[2] = 0x00;
	event->dyndata.data[3] = g_battery;
	event->dyndata.data[4] = charge_state;
	event->dyndata.data[5] = 0x00;
	event->dyndata.data[6] = 0x00;
	event->dyndata.data[7] = 0x00;
	event->dyndata.data[8] = 0x00;
	event->dyndata.data[9] = BAT_CAP_INFO_LOW_BYTE;
	event->dyndata.data[10] = BAT_CAP_INFO_HIGH_BYTE;
	...
	//sys_put_le16(rd->items.item[idx].usage_id,  &event->dyndata.data[sizeof(rs->report_id) + 1]);
    LOG_HEXDUMP_INF(event->dyndata.data, event->dyndata.size,__func__);
	APP_EVENT_SUBMIT(event);
...
#ifdef CONFIG_DPM_ENABLE
        if (rs->report_id == REPORT_ID_DPM_INPUT)
            pipeline_depth = 1;
#endif
...
static void update_key(const struct hid_keymap *map, int16_t value)
{
    ...
    LOG_DBG("%s ID=%d, data=0x%04X",__func__, report_id, map->usage_id);
...
#ifdef CONFIG_DPM_ENABLE
    if (IS_ENABLED(CONFIG_DPM_ENABLE)) {
		report_data_index[REPORT_ID_DPM_INPUT] = data_id;
		report_state_index[REPORT_ID_DPM_INPUT] = state_id;

		state.report_data[data_id].items.item_count_max = DPM_INPUT_REPORT_COUNT_MAX;
...
#if CONFIG_DESKTOP_TILT_BUTTONS_ENABLE
static bool handle_tilt_event(const struct tilt_event *event)
{
	if (!IS_ENABLED(CONFIG_DESKTOP_HID_REPORT_MOUSE_SUPPORT)) {
		return false;
	}
	...
	struct report_data *rd = get_report_data(REPORT_ID_MOUSE);
	__ASSERT_NO_MSG(rd != NULL);
	rd->axes.axis[MOUSE_REPORT_AXIS_TILT] = event->tilt;
	report_send(NULL, rd, true, true);
...
static bool battery_state_level_event_handler(const struct battery_state_event *event)
{ 	 
	struct report_data *rd = get_report_data(REPORT_ID_DPM_INPUT);
	__ASSERT_NO_MSG(rd != NULL);	
	report_send(NULL, rd, true, true);
...
static bool app_event_handler(const struct app_event_header *aeh)
{
...
#if CONFIG_DESKTOP_TILT_BUTTONS_ENABLE
	if (!IS_ENABLED(CONFIG_DESKTOP_TILT_NONE) &&
	    is_tilt_event(aeh)) {
		return handle_tilt_event(cast_tilt_event(aeh));
	}
#endif
	...
	if (is_battery_state_event(aeh) ) {
		...
		return battery_state_level_event_handler(cast_battery_state_event(aeh));
	}
	if ( is_battery_level_event(aeh)) {
		
		return battery_state_level_event_handler(cast_battery_level_event(aeh));
	}
#endif
...
#if (CONFIG_CY25KB_RECHARGEABLE)
APP_EVENT_SUBSCRIBE(MODULE, battery_state_event);//aw
APP_EVENT_SUBSCRIBE(MODULE, battery_level_event);//aw
#endif
#if CONFIG_DESKTOP_TILT_BUTTONS_ENABLE
APP_EVENT_SUBSCRIBE(MODULE, tilt_event);
#endif
