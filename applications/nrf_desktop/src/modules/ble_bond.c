...
static const struct state_switch state_switch[] = {
	...
	{STATE_IDLE,              CLICK_SHORT,  STATE_IDLE,       		 select_confirm},
#endif
 ...
 * peer IDs.
 */
// static uint8_t bt_stack_id_lut[BT_STACK_ID_LUT_SIZE];///bill
uint8_t bt_stack_id_lut[BT_STACK_ID_LUT_SIZE];//bill
...
static void switch_peer(void)
{
	...
	event->op = PEER_OPERATION_SWITCH;
	event->bt_app_id = get_app_id();
	event->bt_stack_id = get_bt_stack_peer_id(get_app_id());
...
#endif
#if (CONFIG_DESKTOP_CY25_BLE_PEER_SELECT && CONFIG_MULTILINK_ENABLE)
extern bool is_multi_link_selected;
#endif
...
static void select_confirm(void)
{
	...
	LOG_INF("Select peer");
#else	
	LOG_INF("\033[1;31mselect peer\033[0m");
    switch_peer();
	tmp_peer_id = next_peer_id(cur_ble_peer_id);
#endif
