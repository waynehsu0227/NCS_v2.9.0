...
::West build
SET NAME=cy26ms_tropic_multilink_poc3
SET BUILD_FOLDER=cy26ms_tropic_multi_poc3_l05
SET OVERLAY=cy26ms_tropic_poc3_nrf54l05
SET BOARD=nrf54l15dk/nrf54l05/cpuapp
west build -p -b %BOARD% -d build_%BUILD_FOLDER% -- -DCONF_FILE=prj_%NAME%.conf -DDTC_OVERLAY_FILE=%OVERLAY%.overlay -DFILE_SUFFIX=%NAME%

if not exist hex mkdir hex
if not exist hex\full mkdir hex\full
