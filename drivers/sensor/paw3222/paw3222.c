...
/* CPI */
#define PAW3222_CPI_STEP		30u  //CPI_X Step = 30u, CPI_Y_STEP = 29u
#define PAW3222_CPI_MIN			(0x0C * PAW3222_CPI_STEP)  //12 * 30u = 360 (CPI_X) , 12 * 29u = 348 (CPI_Y)
#define PAW3222_CPI_MAX			(0x86 * PAW3222_CPI_STEP)  //134 * 30u = 4020 (CPI_X) , 134 * 29u = 3886 (CPI_Y)
...
static int paw3222_async_init_low_voltage(const struct device *dev);
...
static int (* const async_init_fn[ASYNC_INIT_STEP_COUNT])(const struct device *dev) = {
	...
	[ASYNC_INIT_STEP_LOW_VOLTAGE] = paw3222_async_init_low_voltage,
...
static int paw3222_async_init_low_voltage(const struct device *dev)
{
	...
	if (!err) {
		...
		err = reg_write(dev, 0x0D, 0x23);
		err = reg_write(dev, 0x0E, 0x24);
		err = reg_write(dev, 0x19, 0x04);
		err = reg_write(dev, 0x2B, 0x6D);
		err = reg_write(dev, 0x30, 0x2E);
		err = reg_write(dev, 0x4B, 0x04);
		err = reg_write(dev, 0x5C, 0xD4);
		err = reg_write(dev, 0x7F, 0x01);
		err = reg_write(dev, 0x06, 0x14);
		err = reg_write(dev, 0x31, 0x25);
		err = reg_write(dev, 0x34, 0xC4);
		err = reg_write(dev, 0x36, 0xCC);
		err = reg_write(dev, 0x37, 0x42);
		err = reg_write(dev, 0x38, 0x01);
		err = reg_write(dev, 0x3A, 0x76);
		err = reg_write(dev, 0x3B, 0x34);
		err = reg_write(dev, 0x42, 0x39);
		err = reg_write(dev, 0x43, 0xF2);
		err = reg_write(dev, 0x44, 0x39);
		err = reg_write(dev, 0x45, 0xF0);
		err = reg_write(dev, 0x46, 0x12);
		err = reg_write(dev, 0x47, 0x39);
		err = reg_write(dev, 0x48, 0xE3);
		err = reg_write(dev, 0x49, 0x48);
		err = reg_write(dev, 0x4A, 0xD3);
		err = reg_write(dev, 0x4B, 0x98);
		err = reg_write(dev, 0x64, 0x46);
		err = reg_write(dev, 0x71, 0x28);
		err = reg_write(dev, 0x72, 0x28);
		err = reg_write(dev, 0x7F, 0x00);
		err = reg_write(dev, 0x09, 0x00);
	}
	if (!err) {
		err = reg_write(dev, PAW3222_REG_WRITE_PROTECT, 0);
	}
