...
extern uint8_t recoonect_delay_fn_time;
extern bool is_smart_connect_active;
//bill
static bool handle_ble_peer_event(const struct ble_peer_event *event)
{
	...
	case PEER_STATE_DISCONNECTED:
		...
		//bill
		if(disconnect_from_dpm==true)
		{
			disconnect_from_dpm=false;
			k_sleep(K_SECONDS(recoonect_delay_fn_time));
			state=STATE_DELAYED_ACTIVE;
			k_work_submit(&adv_delayed_start);
		}
		//bill
		if (state != STATE_OFF) {
			//bill smart connect v2.0
			if(is_smart_connect_active==true)//if(event->reason == BT_HCI_ERR_LOCALHOST_TERM_CONN)//ble mode idle detected
			{
				__NOP();
			__NOP();
			__NOP();
				return false;//stop adv and trigger rf scan when idle detected, this is processed by multi_link_ble_selector.c
			}
			else{
			//bill smart connect v2.0
			__NOP();
