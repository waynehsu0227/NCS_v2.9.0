...
#elif defined(CONFIG_LED_DRIVER) && defined(CONFIG_LP5012)
  #define DT_DRV_COMPAT ti_lp50xx
#elif defined(CONFIG_LED_DRIVER) && defined(CONFIG_MATRIX_SCAN_LED)
  #define DT_DRV_COMPAT matrix_scan_leds
#else
  #error "LED driver must be specified in configuration"
#endif

#ifndef CONFIG_LED_DRIVER
...
#else
static struct led leds[CONFIG_LED_DRIVER_MAX_LEDS];
#endif
...
static int set_color_one_channel(struct led *led, struct led_color *color)
{
	...
	return led_set_brightness(led->dev, LED_ID(led), brightness);
#else
...
static int leds_init(void)
{
...
#ifdef CONFIG_LED_DRIVER
    const struct device *led_dev = DEVICE_DT_GET_ANY(DT_DRV_COMPAT);
    LOG_DBG("led count=%d", ARRAY_SIZE(leds));
#endif
	for (size_t i = 0; (i < ARRAY_SIZE(leds)) && !err; i++) {
        ...
        led->dev = led_dev;
        led->color_count = 1;
        LOG_DBG("led color_count=%d", led->color_count);
#endif
...
#if defined(CONFIG_PM_DEVICE) && !defined(CONFIG_CAF_LEDS_GPIO) && !defined(CONFIG_LED_DRIVER)
