...
size_t power_down_interval_counter;//bill smart connect v2.0
...
extern bool is_smart_connect_active;//bill , if true, no multilink is connected
...
#if 1
	   	NRF_P1->PIN_CNF[0] = 2; //ZA
	   	NRF_P1->PIN_CNF[1] = 2; //ZB
	   	NRF_P1->PIN_CNF[2] = 2; //PWR_LED
	   	NRF_P1->PIN_CNF[3] = 2; //RF_LED
	   	NRF_P1->PIN_CNF[4] = 2; //I2C_SL1
	   	NRF_P1->PIN_CNF[5] = 2; //BT2_LED
	   	NRF_P1->PIN_CNF[7] = 2; //BT1_LED

	   	NRF_P2->PIN_CNF[2] = 2; //MOSI
	   	NRF_P2->PIN_CNF[4] = 2; //MISO
	   	NRF_P2->PIN_CNF[6] = 2; //SCK
#endif
		for(uint8_t i=0;i<=4;i++)
		{
			LOG_ERR("P0.%d[0x%x]",i ,NRF_P0->PIN_CNF[i]);
		}
		for(uint8_t i=0;i<=14;i++)
		{
			LOG_ERR("P1.%d[0x%x]",i ,NRF_P1->PIN_CNF[i]);
		}
		for(uint8_t i=0;i<=10;i++)
		{
			LOG_ERR("P2.%d[0x%x]",i ,NRF_P2->PIN_CNF[i]);
		}
#endif
	system_off_post_action();
}
...
extern enum protocol last_used_channel;
void smart_connect_power_down(void)
{
...
static void power_down(struct k_work *work)
{
	if (keep_alive_flag) {
		...
		if(is_smart_connect_active==false)//bill , if true, no multilink is connected
		{
			power_down_counter_reset();
		}
		else
	...
	smart_connect_power_down();//bill smart connect process power down
...
//bill
void restart_power_down_counter(void)
{
	k_work_cancel_delayable(&power_down_trigger);
			k_work_reschedule(&power_down_trigger, POWER_DOWN_CHECK_INTERVAL);
}
...
// 		if(is_smart_connect_active)
// 		{
// 			k_work_cancel(&power_down_trigger);
// 			k_work_reschedule(&power_down_trigger, POWER_DOWN_CHECK_INTERVAL);
// 		}
// 	}
// }
...
static bool app_event_handler(const struct app_event_header *aeh)
{
	...
	if (IS_ENABLED(CONFIG_CAF_KEEP_ALIVE_EVENTS) && is_keep_alive_event(aeh)) {
		//bill prevent ble can't enter power down mode
		if(NRF_POWER_S->GPREGRET[1]!=0)
		{
			power_down_interval_counter=NRF_POWER_S->GPREGRET[1]&0x7F;
			NRF_POWER_S->GPREGRET[1]=0;
		}
		else
		{
