...
#define SCAN_TIME_REDUCE (CONFIG_CY25KB_RECHARGEABLE || CONFIG_CY25KB_DRYCELL)
#define FORCE_DISCHARGE 1
...
//#if IS_ENABLED(CONFIG_SOC_NRF54L15) || IS_ENABLED(CONFIG_SOC_NRF54L10  || IS_ENABLED(CONFIG_SOC_NRF54L05 )  )
...
#if SCAN_TIME_REDUCE
static int set_col_active(uint32_t index)
{
	int err = gpio_pin_configure(gpio_devs[col[index].port],
					col[index].pin, GPIO_OUTPUT);
	if (!err) {
		err = gpio_pin_set_raw(gpio_devs[col[index].port],
						col[index].pin,
						!IS_ENABLED(CONFIG_CAF_BUTTONS_POLARITY_INVERSED));
	}
	if (err) {
		LOG_ERR("Cannot set pin");
		return -EFAULT;
...
static int set_col_inactive(uint32_t index)
{
#if !FORCE_DISCHARGE
	gpio_flags_t flags = GPIO_INPUT;
	gpio_flags_t pull = (IS_ENABLED(CONFIG_CAF_BUTTONS_POLARITY_INVERSED) ?
	...
	int err = gpio_pin_configure(gpio_devs[col[index].port], col[index].pin, flags);

	if (err) {
		LOG_ERR("Cannot set pin input pull");
		return -EFAULT;
	}
#else
	int err = gpio_pin_set_raw(gpio_devs[col[index].port],
						col[index].pin,
						IS_ENABLED(CONFIG_CAF_BUTTONS_POLARITY_INVERSED));
#endif
//	k_busy_wait(1000);
#if FORCE_DISCHARGE
	err = gpio_pin_configure(gpio_devs[col[index].port], col[index].pin, GPIO_INPUT);

	if (err) {
		LOG_ERR("Cannot set pin input");
		return -EFAULT;
	}
#endif
	return 0;
